...
Route::prefix('admin')->middleware(['admin'])->group(function () {
    ...
    Route::get('/pending-payments', [AdminRegistrationController::class, 'pendingPayments'])->name('pending-payments');
...
// ==========================================
// EMAIL PREVIEW ROUTES (For testing in browser)
// ==========================================
Route::prefix('email-preview')->group(function () {
    // 1. Delegate Submission Confirmation (with Acknowledgement ID)
    Route::get('submission/{id?}', function ($id = null) {
        $registration = \App\Models\Registration::with(['user', 'delegateCategory', 'country', 'state', 'latestPayment'])
            ->when($id, fn($q) => $q->where('id', $id)->orWhere('acknowledgement_id', $id)->orWhere('registration_number', $id))
            ->latest()
            ->first();
        ...
        if (!$registration) {
            $user = new \App\Models\User([
                'prefix' => 'Dr.',
                'full_name' => 'Demo Delegate',
                'email' => 'delegate@example.com',
                'mobile_number' => '+91 9876543210',
                'delegate_type' => 'Indian',
            ]);
            $cat = new \App\Models\DelegateCategory([
                'category_name' => 'IPHA Member',
                'indian_fee' => 7000.00,
            ]);
            $country = new \App\Models\Country(['country_name' => 'India']);
            $state = new \App\Models\State(['state_name' => 'Jharkhand']);
            $payment = new \App\Models\Payment([
                'transaction_id' => 'TXN_DEMO_894102',
                'payment_method' => 'QR_Code',
                'total_amount' => 8260.00,
            ]);
            $registration = new \App\Models\Registration([
                'acknowledgement_id' => '84920153',
                'delegate_type' => 'Indian',
            ...
            $registration->setRelation('delegateCategory', $cat);
            $registration->setRelation('country', $country);
            $registration->setRelation('state', $state);
            $registration->setRelation('latestPayment', $payment);
        }

        return view('emails.delegate_submission_confirmation', [
            'registration' => $registration,
            'user'         => $registration->user,
            'payment'      => $registration->latestPayment,
        ]);
    })->name('email.preview.submission');

    // 2. Delegate Registration Confirmation / Approved Receipt
    Route::get('confirmation/{id?}', function ($id = null) {
        ...
        if (!$registration) {
            ...
            $registration = new \App\Models\Registration([
                'acknowledgement_id' => '84920153',
                'registration_number' => 'IND-2027-0123',
                'delegate_type' => 'Indian',
                'status' => 'Approved',
                'accompanying_persons' => 0,
                'participate_in_cme' => false,
        ...
        return view('emails.registration_confirmation', [
            'registration'   => $registration,
            'registrationID' => $registration->registration_number ?? $registration->acknowledgement_id,
        ]);
    })->name('email.preview.confirmation');

    // 3. Abstract Submission Confirmation
    Route::get('abstract/{id?}', function ($id = null) {
        $abstract = \App\Models\AbstractSubmission::with(['user'])
            ->when($id, fn($q) => $q->where('id', $id)->orWhere('acknowledgement_id', $id))
            ->latest()
            ->first();
        ...
        if (!$abstract) {
            ...
            $abstract = new \App\Models\AbstractSubmission([
                'acknowledgement_id'      => 'ABS-ORAL-2027-001',
                'abstract_title'          => 'Public Health Interventions in Rural Communities: A Cross-Sectional Study',
                'presentation_mode'       => 'Oral',
                'presenter_category'      => 'IPHA Member',
                'conference_theme'        => 'Universal Health Coverage',
                'presenting_author_name'  => 'Dr. Demo Author',
                'presenting_author_email' => 'author@example.com',
                'status'                  => 'Submitted',
            ]);
            $abstract->setRelation('user', $user);
        }

        return view('emails.abstract_submission_confirmation', [
            'abstract' => $abstract,
            'user'     => $abstract->user,
        ]);
    })->name('email.preview.abstract');
});
