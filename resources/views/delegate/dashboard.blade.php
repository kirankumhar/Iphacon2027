<x-delegate.layout>
    ...
    <div class="container py-3">
        ...
        @if ($abstract)
            <div class="card border-0 shadow-sm mb-4.5 overflow-hidden" style="border-radius: 16px; border-left: 5px solid #FF6B00 !important; box-shadow: 0 8px 24px rgba(255, 107, 0, 0.08) !important;">
                <div class="card-header bg-white py-3.5 px-3 px-md-4 border-bottom d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <div class="d-flex align-items-center gap-3">
                        <div class="rounded-circle bg-warning bg-opacity-15 text-warning p-2 d-flex align-items-center justify-content-center" style="width: 42px; height: 42px; color: #FF6B00 !important; background: #FFF3E0;">
                            <i class="fas fa-file-invoice fs-5"></i>
                        </div>
                        <div>
                            <h6 class="fw-bold text-dark mb-0" style="font-size: 1.05rem;"><i class="fas fa-clipboard-check text-primary me-1.5"></i>Submitted Abstract Details</h6>
                    ...
                    <div>
                        @if ($abstract->status === 'Submitted' || !empty($abstract->acknowledgement_id))
                            <span class="badge bg-success text-white px-3.5 py-1.5 rounded-pill fw-bold" style="font-size: 0.78rem; background-color: #10B981 !important;">
                                <i class="fas fa-check-circle me-1"></i>SUBMITTED
                            </span>
                        @else
                            <span class="badge bg-warning text-dark px-3.5 py-1.5 rounded-pill fw-bold" style="font-size: 0.78rem;">
                ...
                <div class="card-body p-3.5 p-md-4">
                    <h5 class="fw-bold text-primary mb-3" style="letter-spacing: -0.3px; font-size: 1.2rem;">
                    ...
                    <div class="row g-3 mb-3.5">
                        <div class="col-md-4">
                            ...
                            <span class="badge bg-light border text-dark fw-bold px-3 py-1.5 rounded-3" style="font-size: 0.82rem;">
                                <i class="fas fa-microphone-alt text-primary me-1.5"></i>{{ $abstract->presentation_mode ?? 'Not Specified' }}
                            </span>
                        </div>
                        <div class="col-md-4">
                            <span class="text-muted extra-small d-block mb-1">Presenter Category</span>
                            <span class="badge bg-light border text-dark fw-bold px-3 py-1.5 rounded-3" style="font-size: 0.82rem;">
                                <i class="fas fa-user-tag text-info me-1.5"></i>{{ $abstract->presenter_category ?? 'N/A' }}
                            </span>
                        </div>
                        <div class="col-md-4">
                            <span class="text-muted extra-small d-block mb-1">Presenting Author</span>
                            <span class="fw-bold text-dark small">
                                <i class="fas fa-user text-secondary me-1.5"></i>{{ $abstract->presenting_author_name }}
                            </span>
                        </div>
                    </div>
                    ...
                    @if(!empty($abstract->conference_theme))
                        <div class="mb-3.5 p-3 bg-light rounded-3 border">
                            ...
                            <span class="fw-bold text-dark small"><i class="fas fa-tag text-warning me-1.5"></i>{{ $abstract->conference_theme }}</span>
                        </div>
                    @endif

                    @if(!empty($abstract->keywords))
                        <div class="mb-3.5">
                            <span class="text-muted extra-small d-block mb-1.5">Keywords:</span>
                            <div class="d-flex gap-1.5 flex-wrap">
                                @foreach(explode(',', $abstract->keywords) as $kw)
                                    @if(trim($kw))
                                        <span class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 px-3 py-1.5 rounded-3" style="font-size: 0.78rem;">
                                            #{{ trim($kw) }}
                                        </span>
                                    @endif
                                @endforeach
                    ...
                    <div class="d-flex justify-content-between align-items-center pt-3 border-top flex-wrap gap-2">
                        ...
                        <a href="{{ route('abstract.create') }}" class="btn btn-sm btn-primary btn-capsule px-4 py-2 shadow-xs" style="background: linear-gradient(135deg, #013069, #0d47a1); border: none;">
                            <i class="fas fa-external-link-alt me-1.5"></i>View Full Abstract Details
                        </a>
                    </div>
                </div>
            </div>
