...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="d-flex align-items-center justify-content-between py-3 mb-3">
        <h5 class="mb-0"><span class="invert-text-white"><i class="bx bx-globe me-2 text-info"></i>International Approved Registrations</span></h5>
        <span class="badge bg-success rounded-pill px-3 py-2 fs-7 fw-bold shadow-xs">
    ...
    <div class="card border-0 shadow-sm rounded-3">
        <div class="card-header bg-white py-3 border-bottom d-flex align-items-center justify-content-between flex-wrap gap-2">
            <h6 class="mb-0 fw-bold text-dark"><i class="bx bx-list-check me-1.5 text-primary"></i>Approved International Delegates List</h6>
            <div class="search-box" style="max-width: 300px; width: 100%;">
                <div class="input-group input-group-sm">
                    <span class="input-group-text bg-light border-end-0"><i class="bx bx-search text-muted"></i></span>
                    <input type="text" id="approvedIntSearchInput" class="form-control bg-light border-start-0" placeholder="Search by name, reg no, email...">
                </div>
            </div>
        </div>
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" id="approvedIntDelegatesTable">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th class="pe-3 py-3 text-end" style="width: 15%;">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-2.5">
                                        ...
                                        <div>
                                            <h6 class="mb-0 fw-bold text-dark">{{ $reg->user?->prefix }} {{ $reg->user?->full_name }}</h6>
                                ...
                                <td>
                                    <span class="badge bg-label-info px-2.5 py-1 mb-1 fw-semibold extra-small">{{ $reg->delegate_type ?? 'International' }} Delegate</span>
                                    <div class="fw-bold text-dark extra-small">
                                ...
                                <td>
                                    <span class="badge bg-success px-3 py-1.5 rounded-pill fw-bold text-uppercase shadow-xs" style="font-size: 0.75rem;">
                                ...
                                <td class="pe-3 text-end">
                                    <div class="d-inline-flex gap-1.5 align-items-center">
                                        ...
                                        @if($reg->registration_number)
                                            <a href="{{ route('download.receipt', $reg->registration_number) }}" target="_blank" class="btn btn-sm btn-outline-danger py-1 px-2 fw-semibold rounded-2" title="Download Receipt PDF">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-5">
                                    <div class="py-4">
                                        <i class="bx bx-globe text-info mb-2" style="font-size: 3.5rem;"></i>
                                        <h6 class="fw-bold text-dark mb-1">No Approved International Registrations</h6>
                                        <p class="text-muted small mb-0">Approved foreign delegate registrations will appear here once verified.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
...
<script>
    $(document).ready(function() {
        $('#approvedIntSearchInput').on('keyup', function() {
            var value = $(this).val().toLowerCase();
            $('#approvedIntDelegatesTable tbody tr').filter(function() {
