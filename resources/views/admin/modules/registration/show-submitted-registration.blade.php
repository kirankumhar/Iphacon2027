        ...
        <div class="card-header bg-white py-3 border-bottom d-flex align-items-center justify-content-between flex-wrap gap-2">
            ...
            <div class="search-box" style="max-width: 280px; width: 100%;">
                <div class="input-group input-group-sm">
                    ...
                    <input type="text" id="submittedSearchInput" class="form-control bg-light border-start-0" placeholder="Search name, ack id, email...">
                </div>
            </div>
        </div>
                ...
                <table class="table table-hover align-middle mb-0" id="submittedDelegatesTable">
                    <thead style="background-color: #F8FAFC; border-bottom: 1px solid #E2E8F0;">
                        <tr>
                            ...
                            <th class="py-3 text-secondary text-uppercase fw-bold extra-small" style="width: 22%;"><i class="bx bx-category text-info me-1"></i> Category &amp; Type</th>
                            <th class="py-3 text-secondary text-uppercase fw-bold extra-small" style="width: 20%;"><i class="bx bx-credit-card text-success me-1"></i> Payment Info</th>
                            ...
                            <th class="pe-3 py-3 text-secondary text-uppercase fw-bold text-end extra-small" style="width: 17%;"><i class="bx bx-slider-alt text-warning me-1"></i> Actions</th>
                        </tr>
                    </thead>
                                ...
                                <td class="pe-3 text-end">
                                    <div class="d-flex align-items-center justify-content-end gap-1" style="white-space: nowrap;">
                                        ...
                                        <form method="POST" action="{{ route('student-approved-regis') }}" class="d-inline m-0">
                                            ...
                                            <button type="submit" class="btn btn-xs btn-success fw-bold px-2.5 py-1 rounded-2 shadow-xs d-inline-flex align-items-center gap-1" style="font-size: 0.74rem;" onclick="return confirm('Approve registration for {{ $reg->user?->full_name }}?')" title="Approve Registration">
                                                ...
                                                <form method="POST" action="{{ route('student-revert-regis') }}">
                                                    ...
                                                    <div class="modal-header bg-warning bg-opacity-10 py-3 border-bottom-0">
                                                        <div class="d-flex align-items-center gap-2">
                                                            ...
                                                            <h5 class="modal-title fw-bold text-dark mb-0">Revert Registration</h5>
                                                        </div>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                    </div>
                                                    <div class="modal-body py-3">
                                                        <div class="alert alert-warning border-0 extra-small mb-3 d-flex align-items-start gap-2">
                                                            <i class="bx bx-info-circle fs-5 flex-shrink-0 mt-0.5"></i>
                                                            <div>Reverting will allow delegate <strong>{{ $reg->user?->full_name }}</strong> to make corrections or re-upload payment screenshot.</div>
                                                        </div>
                                                        <div class="mb-3">
                                                            <label class="form-label fw-semibold extra-small text-dark">Revert Reason / Remarks *</label>
                                                            <textarea name="reason" class="form-control form-control-sm" rows="3" placeholder="Enter reason for reverting..." required></textarea>
                                                        </div>
                                                    </div>
                                                    <div class="modal-footer border-top-0 pt-0">
                                                        <button type="button" class="btn btn-sm btn-label-secondary" data-bs-dismiss="modal">Cancel</button>
                                                        <button type="submit" class="btn btn-sm btn-warning text-dark fw-bold px-3"><i class="bx bx-send me-1"></i> Submit Revert</button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
                                        ...
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content border-0 shadow-lg rounded-3">
                                                <form method="POST" action="{{ route('student-reject-regis') }}">
                                                    ...
                                                    <input type="hidden" name="registration_id" value="{{ $reg->id }}">
                                                    <input type="hidden" name="acknowledgement_id" value="{{ $reg->acknowledgement_id }}">
                                                    <div class="modal-header bg-danger bg-opacity-10 py-3 border-bottom-0">
                                                        <div class="d-flex align-items-center gap-2">
                                                            ...
                                                            <h5 class="modal-title fw-bold text-danger mb-0">Reject Registration</h5>
                                                        </div>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                    </div>
                                                    <div class="modal-body py-3">
                                                        <div class="alert alert-danger border-0 extra-small mb-3 d-flex align-items-start gap-2">
                                                            <i class="bx bx-error-circle fs-5 flex-shrink-0 mt-0.5"></i>
                                                            <div>Are you sure you want to reject registration for <strong>{{ $reg->user?->full_name }}</strong>?</div>
                                                        </div>
                                                        <div class="mb-3">
                                                            <label class="form-label fw-semibold extra-small text-dark">Rejection Reason *</label>
                                                            <textarea name="reason" class="form-control form-control-sm" rows="3" placeholder="Enter reason for rejection..." required></textarea>
                                                        </div>
                                                    </div>
                                                    <div class="modal-footer border-top-0 pt-0">
                                                        <button type="button" class="btn btn-sm btn-label-secondary" data-bs-dismiss="modal">Cancel</button>
                                                        <button type="submit" class="btn btn-sm btn-danger fw-bold px-3"><i class="bx bx-x me-1"></i> Confirm Reject</button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-5">
                                    <div class="py-4 text-muted">
                                        ...
                                        <p class="mb-0 extra-small">No submitted registrations pending approval at this moment.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
