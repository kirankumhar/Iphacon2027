@extends('admin.layouts.main')

@section('admin-content')
<div class="container-xxl flex-grow-1 mt-3.5 mb-4">
    <div class="d-flex align-items-center justify-content-between py-2 mb-3">
        <h5 class="mb-0 fw-bold text-dark">
            <i class="bx bx-x-circle me-2 text-danger fs-4"></i>Failed Payments
        </h5>
        <span class="badge bg-danger text-white rounded-pill px-3.5 py-2 fs-7 fw-bold shadow-xs">
            Total Failed: {{ $payments->count() }}
        </span>
    </div>

    <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
        <div class="card-header bg-white py-3 border-bottom d-flex align-items-center justify-content-between flex-wrap gap-2">
            <h6 class="mb-0 fw-bold text-dark"><i class="bx bx-list-check me-1.5 text-danger"></i>Failed Transactions History</h6>
            <div class="search-box" style="max-width: 300px; width: 100%;">
                <div class="input-group input-group-sm">
                    <span class="input-group-text bg-light border-end-0"><i class="bx bx-search text-muted"></i></span>
                    <input type="text" id="failedSearchInput" class="form-control bg-light border-start-0" placeholder="Search txn id, name, reg no...">
                </div>
            </div>
        </div>

        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" id="failedPaymentsTable">
                    <thead class="bg-light">
                        <tr>
                            <th class="ps-3 py-3" style="width: 5%;">#</th>
                            <th class="py-3" style="width: 25%;">Delegate Name</th>
                            <th class="py-3" style="width: 25%;">Transaction ID</th>
                            <th class="py-3" style="width: 15%;">Amount</th>
                            <th class="py-3" style="width: 15%;">Status</th>
                            <th class="pe-3 py-3 text-end" style="width: 15%;">Date</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($payments as $index => $pay)
                            <tr>
                                <td class="ps-3 fw-bold text-muted">{{ $index + 1 }}</td>
                                <td>
                                    <h6 class="mb-0 fw-bold text-dark" style="font-size: 0.9rem;">
                                        {{ $pay->registration?->user?->prefix }} {{ $pay->registration?->user?->full_name ?? 'N/A' }}
                                    </h6>
                                    <small class="text-muted extra-small">
                                        <i class="bx bx-hash me-0.5"></i>{{ $pay->registration?->registration_number ?? 'N/A' }}
                                    </small>
                                </td>
                                <td>
                                    <span class="font-monospace fw-bold text-secondary" style="font-size: 0.85rem;">
                                        {{ $pay->transaction_id ?: ($pay->gateway_transaction_id ?: 'N/A') }}
                                    </span>
                                </td>
                                <td>
                                    <span class="fw-bold text-danger" style="font-size: 0.9rem;">
                                        ₹{{ number_format($pay->total_amount, 2) }}
                                    </span>
                                </td>
                                <td>
                                    <span class="badge bg-danger text-white px-2.5 py-1 rounded-pill fw-bold" style="font-size: 0.72rem;">
                                        {{ strtoupper($pay->payment_status ?: 'FAILED') }}
                                    </span>
                                </td>
                                <td class="pe-3 text-end">
                                    <small class="text-muted extra-small">
                                        {{ $pay->payment_date ? $pay->payment_date->format('d M Y, h:i A') : ($pay->created_at ? $pay->created_at->format('d M Y, h:i A') : '-') }}
                                    </small>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-5">
                                    <div class="text-muted">
                                        <i class="bx bx-check-circle fs-1 mb-2 text-success"></i>
                                        <p class="mb-0 fw-semibold">No failed payment records.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>
@endsection

@push('scripts')
<script>
document.addEventListener('DOMContentLoaded', function() {
    const searchInput = document.getElementById('failedSearchInput');
    const tableRows = document.querySelectorAll('#failedPaymentsTable tbody tr');

    if (searchInput) {
        searchInput.addEventListener('keyup', function() {
            const query = this.value.toLowerCase().trim();
            tableRows.forEach(row => {
                const text = row.innerText.toLowerCase();
                row.style.display = text.includes(query) ? '' : 'none';
            });
        });
    }
});
</script>
@endpush
